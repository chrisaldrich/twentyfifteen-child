...
if ( ! function_exists( 'twentyfifteen_entry_meta' ) ) :
	...
	function twentyfifteen_entry_meta() {
		...
		if ( in_array( get_post_type(), array( 'post', 'attachment' ) ) ) {
			$time_string = '<time class="entry-date dt-published published dt-updated updated" datetime="%1$s">%2$s</time>';

			if ( get_the_time( 'U' ) !== get_the_modified_time( 'U' ) ) {
				$time_string = '<time class="entry-date dt-published published" datetime="%1$s">%2$s</time><time class="dt-updated updated" datetime="%3$s">%4$s</time>';
			}
		...
		if ( 'post' == get_post_type() ) {
			if ( is_singular() || is_multi_author() ) {
				printf(h-card
					'<span class="byline"><span class="u-author author hcard vcard"><span class="screen-reader-text">%1$s </span><a class="p-name u-url url fn n" href="%2$s">%3$s</a></span></span>',
			...
			if ( function_exists( 'get_syndication_links' ) ) {
				twentyfifteen_syndication_links();
			}

			if ( function_exists( 'get_simple_location' ) ) {  //removed conditional: && is_singular() 
				echo '<span class="entry-location">';	
...
*/
function twentyfifteen_syndication_links() {
	$args = array(
		'text' => false,
		'icons' => true,
